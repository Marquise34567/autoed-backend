...
const { admin, db: svcDb, bucket: svcBucket } = require('../services/firebaseAdmin')
...
router.post('/', async (req, res) => {
  try {
    ...
    // Resolve bucket
    let useBucket = svcBucket
    try {
      if (!useBucket && admin && admin.storage) useBucket = admin.storage().bucket(process.env.FIREBASE_STORAGE_BUCKET)
    } catch (e) { console.warn('[upload-url] failed to resolve bucket', e && e.message); useBucket = null }
    ...
    try {
      if (svcDb) {
        ...
        await svcDb.collection('jobs').doc(jobId).set({
          id: jobId,
          uid: null,
          status: 'created',
          progress: 0,
