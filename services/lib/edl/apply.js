...
function sanitizeEDL(edl, duration) {
...
function calculateKeptSeconds(edl) {
...
function validateEDLThresholds(edl, duration) {
...
function repairAgggressiveEDL(edl, duration) {
...
function mergeAndCapSegments(segments, maxSegments = 25) {
    const MIN_SEG = 0.8;
    const MERGE_GAP = 0.4;
...
function buildFilterComplex(parts, soundEnhance, watermark = false, exportQuality) {
    const filters = [];
    ...
    let videoOut = "[v]";
    const targetHeight = exportQuality === "4k" ? 2160 : exportQuality === "1080p" ? 1080 : exportQuality === "720p" ? 720 : undefined;
    if (targetHeight) {
    ...
    if (watermark) {
        filters.push(`${videoOut}drawtext=text='AutoEditor':fontsize=20:fontcolor=white@0.4:x=w-tw-10:y=h-th-10:fontfile=/Windows/Fonts/arial.ttf[vout]`);
        videoOut = "[vout]";
    }
...
async function validateOutputFile(outputPath, inputPath, expectedKeptSec) {
    try {
        ...
        const minSizeForInput = Math.max(5000000, inputSizeBytes * 0.05);
        ...
        let outputDurationSec;
        try {
            const metadata = await (0, ffprobe_1.getVideoMetadata)(outputPath);
...
async function applyEDL(params) {
    const { inputPath, edl, outputPath, jobId, soundEnhance = true, watermark = false, exportQuality, onProgress, } = params;
    try {
        ...
        console.log(`[EDL Apply] Sanitized: hook ${working.hook.start.toFixed(3)}s-${working.hook.end.toFixed(3)}s, ${working.segments.length} segments`);
        const keptSec = calculateKeptSeconds(working);
        const validation = validateEDLThresholds(working, videoDuration);
        console.log(`[EDL Apply] Validation: keptSec=${keptSec.toFixed(2)}s, valid=${validation.valid}, reason='${validation.reason}'`);
        if (!validation.valid) {
        ...
        try {
            await (0, exec_1.runCommandWithProgress)(ffmpeg, args, {
        ...
        const outputStats = fsSync.statSync(outputPath);
        console.log(`[EDL Apply] ✓ Output valid: ${(outputStats.size / 1024 / 1024).toFixed(2)}MB, ${outputDuration.toFixed(2)}s`);
        const usedEdl = {
            ...working,
