...
async function GET(request) {
    try {
        ...
        const plan = (0, plans_1.getPlan)(entitlements.planId); // Use entitlements.planId, not subscription.planId
        // Determine if user can render (based on entitlements)
        const canRender = entitlements.rendersPerMonth >= 999999 ||
            subscription.rendersUsedThisPeriod < entitlements.rendersPerMonth;
        // Calculate period info
        const now = Math.floor(Date.now() / 1000);
        const periodDaysRemaining = Math.max(0, Math.ceil((subscription.currentPeriodEnd - now) / (24 * 60 * 60)));
        const rendersRemaining = Math.max(0, entitlements.rendersPerMonth >= 999999
            ? 999999
        ...
        return server_1.NextResponse.json({
            ...
            periodEndDate: new Date(subscription.currentPeriodEnd * 1000).toISOString(),
            periodDaysRemaining,
            canRender,
            message,
        });
